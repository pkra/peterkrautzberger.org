...
First, modern browsers (IE9+) are good enough to allow you to convert MathML "statically" into HTML or SVG markup so that the result provides the same layout on all browsers, comparable to TeX quality but naturally integrated into the surrounding content. To be clear, this is not (just) about client-side rendering like MathJax.
