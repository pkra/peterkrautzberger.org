...
MathML did not start out not as an XML language but simply as the `<math>` tag in HTML3 (yes, it looked very different but let's not forget this origin). It was the browser vendors (Microsoft and Netscape) who rejected it. So math went into XML exile and returned in HTML5 (along its second-class cousin, SVG).
